...
if(changePwForm != null){ // changePwForm 요소가 존재할 때
    ...
    changePwForm.addEventListener("submit", function(event){
        ...
        if(newPwConfirm.value.trim().length == 0){
            ...
            // newPwConfirm.focus();
            // newPwConfirm.value = "";
            alertAndFocus(newPwConfirm, "새 비밀번호 확인를 입력해주세요.");
            event.preventDefault();
            return;
        }
...
// 3) 1번, 2번이 모두 유효할 때 정말 
//    탈퇴를 진행할 것인지 확인하는 confirm출력
//    (확인 클릭 -> 탈퇴 / 취소 -> 탈퇴 취소)
...
if(memberDeleteForm != null){
    ...
    memberDeleteForm.addEventListener("submit", function(event){
        ...
        if(memberPw.value.trim().length == 0){
            ...
            alertAndFocus(memberPw, "비밀번호를 입력하세요.");
            event.preventDefault();
            return;
        }
...
if(imageInput != null){
    ...
    imageInput.addEventListener("change", e => {
        ...
        // * 화살표 함수에서는 this는 window 객체를 의미해서 사용 불가
