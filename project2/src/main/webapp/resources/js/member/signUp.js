...
memberEmail.addEventListener("input", function(){
    ...
    if(memberEmail.value.trim().length == 0){
        ...
        // confirm, error 클래스 제거 -> 글씨 검정색으로 만들기
...
memberPw.addEventListener("input", function(){
    ...
    // 비밀번호가 입력되지 않은 경우
    if(memberPw.value.trim().lenght == 0){
        pwMessage.innerText = "영어, 숫자, 특수문자( !, @, #, -, _ ) 6 ~ 20 글자 사이로 입력해주세요.";
        memberPw.value = "";

        // 검정 글씨로 변환 ( 기본 스타일로 변경 )
...
memberPwConfirm.addEventListener("input", function(){
    ...
    if(checkObj.memberPw){ // 비밀번호가 유효한 경우
        ...
        if(memberPw.value == memberPwConfirm.value){
            ...
            pwMessage.innerText = "비밀번호 확인이 일치하지 않습니다.";
            pwMessage.classList.add("error");
            pwMessage.classList.remove("confirm");
            checkObj.memberPwConfirm = false;
...
// 닉네임 유효성 검사
const memberNickname = document.getElementById("memberNickname"); // input
const nickMessage = document.getElementById("nickMessage"); // span

memberNickname.addEventListener("input", function(){

    // 닉네임에 문자가 입력되지 않은 경우
    if(memberNickname.value.trim().lenght == 0){
        nickMessage.innerText = "한글, 영어, 숫자로만 2 ~ 10 글자 입력해주세요.";
    ...
    if(regEx.test(memberNickname.value)){ // 유효한 경우
        ...
        $.ajax({
            ...
            data : param,
            Type : "GET", // type 미작성 시 기본값 GET 방식이여서 생략 가능
            success : (res) => {
...
memberTel.addEventListener("input", function(){
    ...
    // 문자가 입력되지 않은 경우
    if(memberTel.value.trim().lenght == 0){
    ...
    // 전화번호 정규 표현식 검사
    const regEx = /^0(1[01679]|2|[3-6][1-5]|70)[1-9]\d{2,3}\d{4,4}$/;
