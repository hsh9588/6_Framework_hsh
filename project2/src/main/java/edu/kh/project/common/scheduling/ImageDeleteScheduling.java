...
public class ImageDeleteScheduling {
    ...
    */
	...
	// @Scheduled(cron = "0,15,30 * * * * *") // 매 분 0초, 15초, 30초마다 수행
	// @Scheduled(cron = "0/15 * * * * *") // 매 분 0초 부터 시작하여 15초마다 수행
	@Scheduled(cron = "0 * * * * *") // 매 분 0초마다
	public void delteImageFile() {
		
		// System.out.println("0초마다 출력");
		
		// 1. DB에서 BOARD_IMG 테이블의 모든 이미지 번경명을 조회
		...
		if(!fileList.isEmpty()) { 
			...
			for(File file : fileList) {
				...
				// /파일명.jpg 이렇게 나오지만 + 1 를 추가 함으로써 "파일명.jsp" 가 된다
				String fileName 
