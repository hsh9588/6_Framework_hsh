...
public class MemberController {
	...
	public String login(HttpServletRequest req) {
		...
		// * forward 방법 : prefix / suffix 를 제외한 나머지 jsp 경로
	...
	// * @RequestParam 생략 하기 *
	// 조건 : 매개변수 이름 == input name속성 값
	
	// 2. @RequestParam 어노테이션 사용
	// - 메서드 매개변수에 전달 받은 파라미터를 주입하는 어노테이션
	
	
	// @RequestParam 생략을 이용해서 짧게 코드 작성 가능하다
	// @RequestMapping(value = "/member/login", method = RequestMethod.POST)
	public String login(@RequestParam("inputEmail") String email, 
			...
			String inputPw, String inputEmail) {
	...
	// * @MedelAttribute 어노테이션 생략도 가능!
	// == 커맨드 객체
	
	// * 참고 *
	// Controller 메서드 매개변수에 객체를 작성하면
	// 자동으로 생성되거나 얻어올 수 있는 이유
	// -> Spring Container 에서 Argument Resolver(매개변수 해결사) 제공
	//	  유연하게 처리함
	
	@PostMapping("/member/login")
	public String login(/* @ModelAttribute */ Member inputMember, Model model, 
			RedirectAttributes ra, @RequestParam(value="saveId", required=false) String saveId, // 체크박스 값 얻어오기
		...
		if(loginMember != null) {
			...
			// addAttribute("Key", Value) == req.setAttribute("Key", Value)
			...
			path = referer; // 로그인 요청전 페이지 주소(referer)
			
			// 로그인 실패 시 "아이디 또는 비밀번호가 일치하지 않습니다." 세션에 추가
			// model.addAttribute("message", "아이디 또는 비밀번호가 일치하지 않습니다.");
			// model의 문제점 -> 메인페이지 주소에 message 값 노출
			// -> RedirectAtrributes로 변환
